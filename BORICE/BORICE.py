...
"""
Module for the estimation of population mean outcrossing rates and inbreeding coefficients
using Bayesian methods.

To use this module on a Mac, please read the following instructions carefully. Before running,
make sure this module is placed on your desktop along with the csv file containing your genotype 
data. In your csv file, missing data should be coded as '-9'. The first row of the csv file should 
contain the following in the first three cells: 1) the number of marker loci, 2) a 0 or 1 to 
indicate the absence or presence of a population name in your data, and 3) a 0 or 1 to indicate the 
absence or presence of a subgroup name in your data. Currently, subgroups are not supported, so if 
subgroups are present they will be ignored. The second row should contain the names of your marker 
loci. All other rows should be in the following format: cell 1 = family name; cell 2 = population 
name (unless there isn't one); cell 3 = allele 1 of marker locus 1; cell 4 = allele 2 of marker locus
1; and so forth for all loci. If maternal individuals are present, they should be indicated with
a '!' after the family name (e.g., 'fam1!').

Now open Terminal and change the working directory to your desktop by entering the following at the 
command prompt and hit enter:

cd /Users/your_user_directory_name_goes_here/Desktop

Once you have successfully changed your working directory to your desktop, enter the following at
the command prompt and hit enter:

python BORICE_for_Mac.py your_csv_file_name.csv

You should then see two BORICE output files on your desktop.
"""
...
class SingleLocusGenotype(object):
	"""A SingleLocusGenotype is an object made up of two alleles, "first" and "second". Alleles
	in a genotype are ordered smallest (first) to largest (second).
	"""
	...
	def calc_prob_offspring_given_selfing_mom_homozygote_standard_model(self, mom_g, locus):
		"""Calculates the probability of a homozygous offspring genotype given selfing and its maternal
		genotype; no null alleles, no allelic drop-out.
		"""
		mf = mom_g.first
	...
	def calc_prob_offspring_given_selfing_mom_heterozygote_standard_model(self, mom_g, locus):
		"""Calculates the probability of a heterozygous offspring genotype given selfing and its maternal
		genotype; no null alleles, no allelic drop-out.
		"""
	...
	def calc_prob_offspring_given_outcrossing_mom_homozygote_standard_model(self, allele_list, allele_freq, mom_g, locus):
		"""Calculates the probability of a homozygous offspring genotype given outcrossing and its maternal
		genotype; no null alleles, no allelic drop-out.
		"""
		mf, ms = mom_g.first, mom_g.second
		sf, ss = self.first, self.second
	...
	def calc_prob_offspring_given_outcrossing_mom_heterozygote_standard_model(self, allele_list, allele_freq, mom_g, locus):
		"""Calculates the probability of a heterozygous offspring genotype given outcrossing and its maternal
		genotype; no null alleles, no allelic drop-out.
		"""
		mf, ms = mom_g.first, mom_g.second
		sf, ss = self.first, self.second
	...
	def impute_new_mom(self, allele_list, allele_freq, inbreeding_coefficient, locus, null_loci):
		"""Selects a new maternal genotype based on allele frequencies if there is no observed genotype at this locus.
		If the maternal genotype is an observed homozygote at this locus, a choice is made whether or not to step
		from the current genotype (homozygote or null heterozygote) based on the probability of those genotypes.
		"""
...
class Individual(object):
	"""An Individual object is a set of multilocus genotypes. Individuals belong to families,
	and are either an offspring or a mom of the family. Individuals also have inbreeding coefficients.
	"""
	...
	def get_imputed_loci(self, population):
		"""Returns a list of genotypes that were imputed by def infer_mom or def tag_mom_genotype,
		and a list of the allele frequencies at each of those loci.
		"""
		imp = []
		alleles = []
		afreq = []
	...
	def calc_prob_offspring_geno(self, outcrossing_rate, population, mom, null_loci):
		"""Calculates an individual's multilocus genotype probability given its single-locus
...
# 		
	...
	def calc_prob_mom_geno(self, population):
		"""Calculates a maternal individual's multilocus genotype probability given its single-locus
		genotype probabilities.
		"""
...
def tag_mom_genotype(momfirst, momsecond, offspring, locus_index, null_loci, family):
	"""Tags an observed maternal genotype as imputed if it is a homozygote, and returns a SingleLocusGenotype.
	This is for the purpose of dealing with null alleles.
	"""
...
class Population(object):
	"""A Population object consists of multiple families. Each population also has a list of
	allele frequencies specific to that population, as well as an outcrossing rate.
	"""
	...
	def calculate_new_af_list(self, locus_alleles, locus_index, step, burn_in, null_loci):
		"""Calculates a new list of allele frequencies at a locus based on the y-value
		of each allele.
		"""
		null = null_loci[locus_index]
		if null:
	...
	def calc_ih_prob(self):
		"""Calculates a list of inbreeding history probabilities based on the
		population outcrossing rate.
		"""
...
class SingleLocusGenotypeError(Exception):
	"""Makes a SingleLocusGenotypeError class. If an impossible genotype is encountered in the
	data, it prints the genotype and identifies the family in which it occurs.
	"""
